...
- **Companion surfaces** - `NookCompanion` and
  `NookConfiguration.addCompanion(...)`; the chrome controls `NookChromeActions`
  (`\.nookChromeActions`), `NookKeepOpenButton`, and `NookSettingsButton`, with the
  `topBar.showsKeepOpenButton` / `showsSettingsButton` flags. See
...
- `NookCompanionSurface`, `NookCompanionAnchor`, `NookCompanionVisibility`,
  ...
  `nookCompanionHidden(_:)` modifiers and `\.nookCompanionIsPresented`. See
