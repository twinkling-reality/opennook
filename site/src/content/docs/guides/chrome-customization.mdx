...
Appearance is kept **field by field**. When the person changes the palette, only the
palette is stored; the surface style, the keep-open lock, and every other field they
never changed keep following your defaults. So a host can move its default surface
style to Liquid Glass in a later build and reach everyone who never picked a style,
even people who turned on the lock. See [Persistence](/guides/theming/#persistence)
for how records saved by earlier builds are migrated.
...
configuration.chromeBehavior = NookChromeBehavior(
    ...
    glassShading: .notchFade,    // default .even: how the framework shades Liquid Glass
    companionBackdrop: nil,      // default nil: what companions inherit
    keyboard: NookKeyboardBehavior(shortcutTakesKeyboardFocus: true)  // default: shortcut only shows
)
```

`glassShading` and `companionBackdrop` are covered in
[Surface materials](/guides/surface-materials/#notch-fade), and `keyboard` in
...
Applied when the surface is built. To change it later - say, from a "haptics on
...
[Playground](/guides/playground/#doing-the-same-in-your-app).
...
`backdrop` overrides how the chrome state maps to the surface backdrop.
...
The resolver receives a `NookBackdropContext`: the current
...
configuration.chromeBehavior = NookChromeBehavior(
    backdrop: { context in
        if context.reduceTransparency || context.preferences.surfaceStyle == .solid {
            return .solid(context.colorScheme == .dark ? .black : .white)
        }
...
#### One chrome, two surfaces

The resolver is re-run on every expand and collapse, so `context.state` is all it
...
`context.form` is the other half of the question - `.auto` resolves to
...
[Surface materials](/guides/surface-materials/#notch-fade).
