---
title: Examples
description: An index of the twelve example apps under Examples/, each linked to the guide that covers it
---

Every example under `Examples/` is a host that builds on OpenNook's public API
only - no forking, no patched framework. All but the playground and the showcase fit in a single
...
- **ChromeNook** - `swift run ChromeNook`. A tour of the deeper chrome seams:
  launch-preference defaults, hover behavior and the notch fade for Liquid Glass,
  a note field the global shortcut types straight into, label and metric
  overrides, a custom brand mark and menu-bar icon, a Settings screen without its
  Data group, top-bar trailing actions, and the status banner. See
  [Chrome customization](/guides/chrome-customization/) and
...
Several seams put together the way a shipping notch app would.

- **ShowcaseNook** - `swift run ShowcaseNook --scene <id>`. Finished-looking
  scenes, one per launch: a player with cover art drawn in code and its queue
  (`player`), a month beside today's schedule (`agenda`), a focus countdown on a
  tick dial (`timer`), a release build step by step that posts an activity when
  it finishes (`progress`), the file shelf holding sample files (`shelf`), a
  volume HUD over `SystemVolumeObserver` (`hud`), and a collapsed pill carrying
  a song and a timer (`compact`). `--expand` or `--expand-after <seconds>` opens
  the nook and `--keep-open` holds it. See
  [Companion surfaces](/guides/companion-surfaces/),
  [Activity queue](/guides/activity-queue/), and
  [File shelf](/guides/file-shelf/).
